...
// Function to generate random characters for watermark font
std::string generate_random_font() {
    std::string font;
    for (int i = 0; i < 6; ++i) {  // Length of custom font string
        // Randomly select a Unicode character
        char random_char = static_cast<char>(rand() % 128 + 33); // ASCII printable characters
        font += random_char;
    }
    return font;
}
